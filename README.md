...
### Technical Workflow
1.  **Data Ingestion** – Automated ingestion and preprocessing of German text via Airflow DAGs.
2.  **Experiment Tracking** – Hyperparameters and ROUGE-L metrics are logged via MLflow to ensure reproducibility and model quality.
3.  **Validation** – Uses a back-translation layer to provide a "semantic logic check" between the source and summary.
4.  **Infrastructure** – Managed repository using **Git LFS** to handle large model weights and binary files.

---

### Learning Outcome
This project demonstrates the successful implementation of a full-scale NLP pipeline, moving beyond simple model training to address production challenges like orchestration, experiment logging, and cross-lingual validation.
